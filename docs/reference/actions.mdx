    ...
    <ResponseField name="stateInit" type="object" required>
      <Expandable title="properties">
        ...
        <ResponseField name="data" type="Map<Uint8Array, Uint8Array>">
          Initial storage key-value pairs (borsh-serialized bytes). Entries are encoded sorted bytewise by key, so insertion order does not affect the derived account ID.
        </ResponseField>
