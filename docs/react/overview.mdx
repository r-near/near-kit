...
function Wallet() {
  ...
  const handleSend = async () => {
    await send({ receiver: "bob.testnet", amount: "1 NEAR" })
  }
...
<Tabs>
  <Tab title="Read Hooks">
    ...
    const { data: account } = useAccount({ accountId: "alice.near" })
    ```
  </Tab>
  <Tab title="Mutation Hooks">
    ...
    // Send tokens
    const { send, isPending } = useSend()
    await send({ receiver: "bob.near", amount: "5 NEAR" })

    // Call contract method
    const { call, isPending } = useCall()
    await call({
