...
console.log(`factory.near publishes ${code.length} bytes, hash ${hash}`)

// Immutable contract by its code hash
const published = await near.getGlobalContract({ codeHash: "5FzD8..." })
```

For updatable contracts, the returned `hash` is the hash of the **current** version — compare it against a locally computed hash to detect when the publisher has updated the code.

To just check existence (for example before `deployFromPublished`):

```typescript
if (await near.globalContractExists({ accountId: "factory.near" })) {
  // safe to deploy by reference
}
```
...
```typescript
import { GlobalContractNotFoundError } from "near-kit"

try {
  await near.getGlobalContract({ accountId: "not-a-publisher.near" })
} catch (error) {
  if (error instanceof GlobalContractNotFoundError) {
    console.log("Nothing published under", error.identifier)
  }
}
```
...
The `data` map is encoded as nearcore's `BTreeMap`, with entries sorted bytewise by key, so the `Map`'s insertion order does not affect the derived ID. Keys must be unique by content.
