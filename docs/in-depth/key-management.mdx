...
const phrase = generateSeedPhrase({ keyType: "ml-dsa-65" }) // 24 words

const pqKey = parseSeedPhrase(phrase, {
  keyType: "ml-dsa-65",
...
A phrase generated *for* ML-DSA-65 must be at least 18 words per [NEP-649](https://github.com/near/NEPs/pull/649) — a 12-word phrase carries 128 bits of entropy, below ML-DSA-65's security level — so `generateSeedPhrase({ keyType: "ml-dsa-65" })` returns 24 words and throws for a `wordCount` of 12 or 15. Recovery is unrestricted: `parseSeedPhrase` derives an ML-DSA-65 key from any valid 12-24 word phrase.

Derivation follows the SLIP-0010 extension proposed in [satoshilabs/slips#1968](https://github.com/satoshilabs/slips/pull/1968) and standardized for NEAR in [NEP-649](https://github.com/near/NEPs/pull/649): the master node is `HMAC-SHA512(key = "ML-DSA-65 seed", data = BIP-39 seed)`, children use the standard SLIP-0010 hardened-only step, and the derived 32-byte node secret is the FIPS 204 seed ξ fed to ML-DSA key generation. Because the master salt differs from ed25519's, the ML-DSA-65 key derived from a phrase is unrelated to the ed25519 key derived from that same phrase.
