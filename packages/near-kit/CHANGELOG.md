...
- ae2a7eb: Update sandbox to nearcore 2.13.4
- fe556b5: Fix secp256k1 signing so nearcore accepts it: sign the 32-byte transaction / delegate / NEP-413 hash directly (no extra SHA-256) and emit `[r][s][v]` instead of `[v][r][s]`. `Secp256k1KeyPair` now also accepts 32-byte secret keys (near-cli / near-crypto format).

## 0.20.1

### Patch Changes

- 316e8ed: Fix NEP-616 deterministic account IDs for state-init data with more than one entry. `deriveAccountId` and the `DeterministicStateInit` action now encode `data` in canonical borsh `BTreeMap` order (sorted bytewise by key), matching nearcore, so multi-entry maps produce the account ID the node expects regardless of insertion order. Two keys with identical bytes now throw instead of yielding an ID the node rejects. Requires `@zorsh/zorsh` 0.5.1, which fixes the underlying map ordering.
